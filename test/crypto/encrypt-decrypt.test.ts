...
describe("encrypt / decrypt", () => {
  ...
  it("deterministic: same content + same keys = same ciphertext", async () => {
    ...
    const getChunks = (s: string) =>
      s
        .split("\n")
        .slice(2)
        .filter((l) => l !== "" && !l.startsWith("seal_b64="));
  ...
  it("empty plaintext roundtrip", async () => {
  ...
  it("previousFile with wrong password falls back to fresh", async () => {
    const text = "hello\n\nworld";
    // Encrypt with same text but using a file encrypted with a different password
    const otherEncrypted = await encrypt(text, WRONG_PASSWORD, opts);
    const result = await encrypt(text, TEST_PASSWORD, {
      ...opts,
      previousFile: otherEncrypted,
    });
    // Should still decrypt correctly (fell back to fresh keys)
    const decrypted = await decrypt(result, TEST_PASSWORD);
    expect(decrypted).toBe(text);
  });

  it("malformed file content rejects on decrypt", async () => {
    await expect(decrypt("not an mdenc file", TEST_PASSWORD)).rejects.toThrow();
  });

  it("truncated file rejects on decrypt", async () => {
    const encrypted = await encrypt("hello\n\nworld", TEST_PASSWORD, opts);
    const lines = encrypted.split("\n");
    // Keep only header and auth line
    const truncated = lines.slice(0, 2).join("\n");
    await expect(decrypt(truncated, TEST_PASSWORD)).rejects.toThrow();
  });

  it("file missing seal rejects on decrypt", async () => {
    const encrypted = await encrypt("hello\n\nworld", TEST_PASSWORD, opts);
    const lines = encrypted.split("\n");
    // Remove the seal line
    const withoutSeal = lines.filter((l) => !l.startsWith("seal_b64=")).join("\n");
    await expect(decrypt(withoutSeal, TEST_PASSWORD)).rejects.toThrow();
  });
});
