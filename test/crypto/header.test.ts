...
import {
  ...
  toBase64,
  verifyHeader,
} from "../../src/crypto/header.js";
import type { MdencHeader } from "../../src/crypto/types.js";
import { DEFAULT_SCRYPT_PARAMS } from "../../src/crypto/types.js";
...
describe("serializeHeader / parseHeader", () => {
  ...
  it("invalid headers produce clear errors", () => {
    ...
    expect(() => parseHeader("mdenc:v1 file_id_b64=AAAA scrypt=N=1024,r=1,p=1")).toThrow(
      "missing salt_b64",
    );
    expect(() =>
      parseHeader("mdenc:v1 salt_b64=AAAAAAAAAAAAAAAAAAAAAA== scrypt=N=1024,r=1,p=1"),
    ).toThrow("missing file_id_b64");
    expect(() =>
      parseHeader(
        "mdenc:v1 salt_b64=AAAAAAAAAAAAAAAAAAAAAA== file_id_b64=AAAAAAAAAAAAAAAAAAAAAA==",
      ),
    ).toThrow("missing scrypt");
  });
  ...
  it("rejects out-of-bounds scrypt parameters", () => {
    ...
    // N too low
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=512,r=1,p=1`),
    ).toThrow("Invalid scrypt N");
    // N too high
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=2000000,r=1,p=1`),
    ).toThrow("Invalid scrypt N");
    // r too low
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=1024,r=0,p=1`),
    ).toThrow("Invalid scrypt r");
    // r too high
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=1024,r=65,p=1`),
    ).toThrow("Invalid scrypt r");
    // p too low
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=1024,r=1,p=0`),
    ).toThrow("Invalid scrypt p");
    // p too high
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=1024,r=1,p=17`),
    ).toThrow("Invalid scrypt p");
  });
  ...
  it("rejects scrypt N that is not a power of 2", () => {
    ...
    const fid = "AAAAAAAAAAAAAAAAAAAAAA==";
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=1025,r=8,p=1`),
    ).toThrow("must be a power of 2");
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=3000,r=8,p=1`),
    ).toThrow("must be a power of 2");
    // Valid powers of 2 should pass
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=1024,r=8,p=1`),
    ).not.toThrow();
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=16384,r=8,p=1`),
    ).not.toThrow();
    expect(() =>
      parseHeader(`mdenc:v1 salt_b64=${salt} file_id_b64=${fid} scrypt=N=1048576,r=8,p=1`),
    ).not.toThrow();
  });
});
...
describe("authenticateHeader / verifyHeader", () => {
  ...
  it("wrong key rejects", () => {
    ...
    expect(verifyHeader(wrongKey, line, mac)).toBe(false);
  });
});

describe("generateSalt / generateFileId", () => {
  it("generateSalt returns 16 bytes", () => {
    const salt = generateSalt();
    expect(salt.length).toBe(16);
  });

  it("generateFileId returns 16 bytes", () => {
  ...
  it("generateFileId produces unique values", () => {
...
describe("toBase64 / fromBase64", () => {
  it("round-trip for arbitrary bytes", () => {
  ...
  it("round-trip for single byte", () => {
    const single = new Uint8Array([0xff]);
    const b64 = toBase64(single);
    const decoded = fromBase64(b64);
