...
import { verifySeal } from "../../src/crypto/seal.js";
...
describe("seal (built into encrypt)", () => {
  ...
  it("decrypt detects replaced chunk", async () => {
    ...
    await expect(decrypt(tampered, TEST_PASSWORD)).rejects.toThrow("Seal verification failed");
  });
  ...
  it("decrypt detects truncation", async () => {
    ...
    await expect(decrypt(truncated, TEST_PASSWORD)).rejects.toThrow("Seal verification failed");
  });
  ...
  it("decrypt detects chunk reorder", async () => {
    ...
    await expect(decrypt(reordered, TEST_PASSWORD)).rejects.toThrow("Seal verification failed");
  });
  ...
  it("decrypt detects header tampering", async () => {
    ...
    await expect(decrypt(tampered, TEST_PASSWORD)).rejects.toThrow("Header authentication failed");
  });
  ...
  it("verifySeal with wrong password throws", async () => {
    const text = "hello\n\nworld";
    const encrypted = await encrypt(text, TEST_PASSWORD, opts);
    await expect(verifySeal(encrypted, "wrong-password")).rejects.toThrow(
      "Header authentication failed",
    );
  });

  it("verifySeal with modified seal line returns false", async () => {
    const text = "hello\n\nworld";
    const encrypted = await encrypt(text, TEST_PASSWORD, opts);
    const lines = encrypted.split("\n");
    const sealIdx = lines.findIndex((l) => l.startsWith("seal_b64="));
    lines[sealIdx] = "seal_b64=AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=";
    const tampered = lines.join("\n");
    const valid = await verifySeal(tampered, TEST_PASSWORD);
    expect(valid).toBe(false);
  });
